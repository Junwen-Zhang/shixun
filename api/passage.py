...
#罗列所有文章列表
@router.get("/getpassages", tags=["passage"]) 
...
@router.get("/getpassages/getpassage", tags=["passage"])
async def searchPassage(pname:str):
    return passageService.searchPassage(pname)

#所有文章->单独文章内容反馈
@router.get("/getpassages/getpassage/{pname}/{pid}", tags=["passage"])
...
#------------------------------------------------------------------------------------------------------------------    
#查看文章评论(pid)
@router.get("/passages/passage/comments/{pname}/{pid}",tags=["passage"])
async def getPassageComment(pname:str,pid:int):
     return passageService.getPassageComment(pid)

#点赞(uid,pid)
@router.post("/passages/passage/liking", tags=["passage"])
...
#------------------------------------------------------------------------------------------------------------------  
#管理员删除文章
@router.delete("/getpassage/", tags=["passage_administrators"])
